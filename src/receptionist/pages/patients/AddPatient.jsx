...
const AddPatient = () => {
  ...
  else if (!phoneRegex.test(data.contact)) errors.contact = "Enter valid 10 digit phone number";
...
const handleInputChange = (e) => {
  ...
  try {
    ...
    ////console.log("Fetched patient details:", response);

    const patient = response?.patient;
    ////console.log("Patient data to populate form:", patient);
    if (!patient) return;

    // Extract doctor ID
    let doctorId = "";
    if (typeof patient.assignedDoctor === "object" && patient.assignedDoctor?._id) {
  ...
  const handleAppointmentSubmit = async (e) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      const payload = {
       ...
       // notes: existingPatientForm.notes || "",
        paymentAmount: existingPatientForm.amount ? parseFloat(existingPatientForm.amount) : 0,
              ...
              >
                ...
                />
                  ...
                  <Select
                    ...
                    options={doctors.map((d) => `${d._id}|||${d.name || d.doctorName}`)}
                  />
            ...
            {activeTab === "existing" && (
              ...
              >
                <Select
                  ...
                  onChange={handleInputChange}
                  options={patients.length > 0 ? patients.map((p) => `${p._id}|||${p.patientName} (${p.contact})`) : []}
                  error={errors.patientId}
                />
                {/* {patients.length === 0 && <p className="text-xs text-red-500">No patients available</p>} */}
...
{/* 
                ...
                <div className="md:col-span-4 flex justify-end mt-4">
                  <button
                    ...
                    className="flex items-center gap-2 bg-[#2D9AD8] text-white px-6 py-2 rounded-md hover:opacity-90 w-full sm:w-auto disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {appointmentMutation.isLoading ? "Booking..." : "+ Add Appointment"}
                  </button>
                </div>
